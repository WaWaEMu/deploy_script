...
echo ""
echo "🔍 Verifying remote consistency..."
echo "------------------------------------------"
...
echo "✅ All remote files verified — ready to deploy."
...
echo ""
echo "💾 Backing up files before deployment..."
echo "------------------------------------------"
...
for FILE in $DIFF_FILES; do
    ...
    if [ -s "$LOCAL_NEW" ]; then
        ...
        scp -P "$SSH_PORT" "$LOCAL_NEW" "$SSH_USER@$SSH_HOST:$REMOTE_FILE" &>/dev/null
    else
        echo "   Removing → $FILE"
